...
(function(window, ko){
	...
	ko.observable.fn.subscribeOnce = ko.computed.fn.subscribeOnce = function( callback, callbackTarget ){
		var wrapper =  function(){
			callback.apply(this, arguments);
			sub.dispose();
		};
		var sub = this.subscribe.call( this, wrapper, callbackTarget );
		return sub;
	};
	...
	var subscribeArray = function( observable, callback, callbackTarget ){
		observable.subscribe( function(oldArray){
			oldArray = oldArray.slice(0);
			var sub = observable.subscribe( function(newArray){
				callback.call( callbackTarget, newArray, oldArray );
				sub.dispose();
			} );
		}, callbackTarget, "beforeChange");
	};

	var arrayKOInitialSync = function(arrayKO, arrayShare){
		arrayShare.forEach(function(value, index){
			arrayKO.setAt(index, arrayShare[index]);
		});
	};
